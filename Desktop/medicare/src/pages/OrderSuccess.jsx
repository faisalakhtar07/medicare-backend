...
import { CheckCircle2, MapPin, Package, MessageCircle } from 'lucide-react'
...
export default function OrderSuccess() {
  ...
  const { orderId, address, total, deliveryMethod, items } = state
  ...
  return (
    <div className="max-w-lg mx-auto px-5 lg:px-6 py-14 text-center">
      ...
      <p className="text-sm text-navy-900/50 mb-8">Your order has been confirmed and is being prepared.</p>
      ...
      <button
        onClick={() => openWhatsApp(buildOrderMessage({ orderId, items: items || [], address, total, deliveryMethod }))}
        className="focus-ring inline-flex items-center gap-1.5 text-sm font-semibold text-mint-600"
      >
        <MessageCircle size={16} /> Resend order on WhatsApp
      </button>
    </div>
  )
}
